...
import { AlertCircle, Link2, FileText, Map } from 'lucide-react';
import { useAppSelector } from '@/store/store';
import { useDispatch } from 'react-redux';
import { setErrorTypes, setIssueRelatedTo, setIssueDescriptions, setSMETemplates } from '@/store/slices/smeConfigSlice';
import { fetchErrorTypes, fetchIssueRelatedTo, fetchIssueDescriptions, fetchSMETemplates } from './settingsApiCalls';
...
import DescriptionMappingSection from './components/DescriptionMappingSection';
import { Skeleton } from '@/components/ui/skeleton';

type SMETabType = 'error-types' | 'issue-related-to' | 'issue-descriptions' | 'description-mapping';
...
const smeTabs = [
  ...
  { id: 'description-mapping' as SMETabType, label: 'Description Mapping', icon: Map },
];
...
const SMEConfigTab: React.FC = () => {
  ...
  const { errorTypesLoaded, issueRelatedToLoaded, issueDescriptionsLoaded, smeTemplatesLoaded } = useAppSelector(state => state.smeConfig);
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const [errorTypes, issueRelatedTo, issueDescriptions, smeTemplates] = await Promise.all([
          fetchErrorTypes(),
          fetchIssueRelatedTo(),
        ...
        dispatch(setSMETemplates(smeTemplates));
      } catch (error) {
  ...
  if (loading && (!errorTypesLoaded || !issueRelatedToLoaded || !issueDescriptionsLoaded || !smeTemplatesLoaded)) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {activeTab === 'description-mapping' && <DescriptionMappingSection />}
      </div>
    </div>
  );
};
